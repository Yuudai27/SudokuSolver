...
public class Layout extends JFrame implements KeyListener{
	...
	private JLabel timeLabel = new JLabel("Solved in: ");
	 ...
	 * @param seconds Takes the number of seconds, which have to be written into the text field.
	 */
	...
	protected int[][][] readInput(){
		...
		for(int x = 0; x < 9; x++) {
			for(int y = 0; y < 3; y++) {
				for(int z = 0; z < 3; z++) {
					if(textFieldArray.get(x*9+y+z*3).getText().isEmpty() == false) {
	...
	private int[] calcCoordinates(int index) {
		...
		result[0] = (index/9)%3;
    	result[1] = (index/9)/3;
    	result[2] = (index-result[0]*9-result[1]*27)%3;
    	result[3] = (index-result[0]*9-result[1]*27)/3;
		return result;
	}
	/**
