...
public class Field {
    ...
    protected void initializeField(int[][] fieldValues) {
        for (int x = 0; x < 3; x++) {
            System.arraycopy(fieldValues[x], 0, fieldNumbers[x], 0, 3);
        }
    }
     ...
     * If there is one the number of the row, column and the number will be copied into the result array to
    ...
    protected int[] checkAllMarks() {
        ...
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                int index = x + y * 3;
                if (fieldNumbers[x][y] == 0 && sizeMarkers[index] == 1) {
                    result[0] = x;
                    result[1] = y;
                    result[2] = markers.get(index).get(0);
                    updateAllMarks(markers.get(index).get(0));
                    return result;
                }
            }
        }
     ...
     * This number will be returned in the result array with row, column and number.
     * The markers for the single field, where the this number is contained, will be cleared
     * and the it will be removed from the other single fields in the field.
     *
    ...
    protected void updateAllMarks(int number) {
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                if (fieldNumbers[x][y] == 0) {
                    int index = x + y * 3;
                    if (markers.get(index).contains(number)) {
                        markers.get(index).remove(Integer.valueOf(number));
                        sizeMarkers[index]--;
                    }
                }
            }
    ...
    protected void updateAllMarksWithException(int xCt, int yCt, int num1, int num2) {
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                if (fieldNumbers[x][y] == 0) {
                    int index = x + y * 3;
                    if (index != xCt && index != yCt) {
                        if (markers.get(index).contains(num1)) {
                            markers.get(index).remove(Integer.valueOf(num1));
                            sizeMarkers[index]--;
                        }
                        if (markers.get(index).contains(num2)) {
                            markers.get(index).remove(Integer.valueOf(num2));
                            sizeMarkers[index]--;
                        }
     ...
     * and removes it in this case from this rows single fields.
     *
    ...
    /**
     * the updateMarksInRow function is checking if the given number exists in the given row
     * and removes it in this case from this rows single fields with exception of the given column.
     *
     * @param number Takes the search number, which has to be removed.
     * @param row    Takes the row and which will be checked.
    ...
    protected void updateMarksInRowWithException(int number, int row, int column) {
        for (int y = 0; y < 3; y++) {
            if (fieldNumbers[row][y] == 0) {
                int index = row + (y * 3);
                if (markers.get(index).contains(number) && y != column) {
                    markers.get(index).remove(Integer.valueOf(number));
                    sizeMarkers[index]--;
                }
            }
     ...
     * and removes it in this case from this columns single fields.
     *
     * @param number Takes the search number, which has to be removed.
    ...
    protected void updateMarksInColumnWithException(int number, int column, int row) {
        for (int x = 0; x < 3; x++) {
            if (fieldNumbers[x][column] == 0) {
                int index = x + column * 3;
                if (markers.get(index).contains(number) && x != row) {
                    markers.get(index).remove(Integer.valueOf(number));
                    sizeMarkers[index]--;
                }
            }
     ...
     * then the second id number, if there exists a second single field with this values.
     * Finally the two possible numbers will also be transfered.
     */
     ...
     * single fields.
     * If these counters are equal, checkedDouble array will be resetted.
     */
    ...
    protected boolean checkDoubleMarksInRow(int pos, int num1, int num2) {
        ...
        for (int y = 0; y < 3; y++) {
            int index = pos + y * 3;
            if (sizeMarkers[index] == 2) {
                if (markers.get(index).contains(num1) && markers.get(index).contains(num2)) {
                    result = true;
                    column = y;
     ...
     *
     * @return The result is a boolean and returns true, if cant be any numbers anymore filled in.
     */
    ...
    /**
     * The printMarkers function is just a test function which prints the possible numbers for each single field.
     */
    protected void printMarkers() {
        for (int x = 0; x < 9; x++) {
            if (sizeMarkers[x] > 0) {
                System.out.println(x);
                for (int y = 0; y < markers.get(x).size(); y++) {
                    System.out.print(markers.get(x).get(y));
                }
                System.out.println();
            }
        }
